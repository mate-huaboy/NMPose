...
#========for tensor================
def crop_resize_by_warp_affine_for_tensor(img, center, scale, output_size, rot=0, interpolation=cv2.INTER_LINEAR):
    """
    output_size: int or (b,c,w, h)
    NOTE: if img is (b,c,h,w)
...
def get_affine_transform_for_tensor(center, scale, rot, output_size, shift=np.array([0, 0], dtype=np.float32), inv=False):
    """
    adapted from CenterNet: https://github.com/xingyizhou/CenterNet/blob/master/src/lib/utils/image.py
    center: ndarray: (cx, cy)
    scale: (w, h)

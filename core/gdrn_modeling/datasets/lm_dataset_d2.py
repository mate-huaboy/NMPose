...
class LM_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.nxyz_prefixes = data_cfg["nxyz_prefixes"] 
    ...
    def __call__(self):  # LM_Dataset   by wenhua very important!!!!!!!!!!!!! get data dict by this func when get datadict list will call
        ...
        dataset_dicts = []  # ###################################################### this is dict contain each data's path
        ...
        mmcv.dump(dataset_dicts, cache_path, protocol=4)   #save the file by wenhua
...
SPLITS_LM = dict(
    lm_13_train=dict(
        ...
        nxyz_prefixes=[osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/nxyz_crop/{:06d}".format(ref.lm_full.obj2id[_obj]))
            for _obj in LM_13_OBJECTS],
    ...
    lm_13_test=dict(
        ...
        nxyz_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/nxyz_crop/{:06d}".format(ref.lm_full.obj2id[_obj]))
            for _obj in LM_13_OBJECTS
        ],
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)

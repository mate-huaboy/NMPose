...
class GDRN_Lite(LightningLite):
    ...
    def do_train(self, cfg, args, model, optimizer, resume=False):
        model.train()   #here what mean by wenhua
        ...
        data_loader = build_gdrn_train_loader(cfg, train_dset_names)  #data input by wenhua
...
>>>>>>> atention
        ...
        with EventStorage(start_iter) as storage:
            for iteration in range(start_iter, max_iter):# start to iteration by wenhua
                ...
                if np.random.rand() < train_2_ratio:
                    ...
                    data = next(data_loader_iter)#this get a batch data actually?
                ...
                # forward ============================================================
                #将data变为需要的格式，例如图片roi_img变为24*64*64
                batch = batch_data(cfg, data) #get a batch data just zuzhi
                # print(data[0]["roi_nxyz"][0][32][30])
                # print(data[0]["roi_nxyz"][1][32][30])
                # print(data[0]["roi_nxyz"][2][32][30])
                

                # nxyz=batch.get("roi_nxyz", None),
                # cpunxyz=nxyz[0].cpu().numpy()
                # print(cpunxyz[30][32][0])
                # print(cpunxyz[30][32][1])
                # print(cpunxyz[30][32][2])
                #查看数据


                out_dict, loss_dict = model(#why require so much reg?
                #也就是前向传播
                    batch["roi_img"],
                    # gt_xyz=batch.get("roi_xyz", None),
                    gt_xyz=batch.get("roi_nxyz", None),
...
>>>>>>> atention
                ...
                self.backward(losses)   #对loss反向传播
                ...
                if (
                    ...
                    self.do_test(cfg, model, epoch=epoch, iteration=iteration)      #test
                ...
                if iteration - start_iter > 5 and (
                    ...
                    if cfg.TRAIN.VIS_IMG:  #when visualize will occu error by wenhua
                        with torch.no_grad():
                            vis_i = 0
                            ...
                            out_xyz_vis = get_emb_show(out_xyz_vis)  #没错，该函数改变值的分别，使得其值大于0，而可以用于图像显示
                            ...
                            if cfg.MODEL.CDPN.ROT_HEAD.ENABLED:

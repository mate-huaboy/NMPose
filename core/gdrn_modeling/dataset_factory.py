...
def get_available_datasets(mod_name):
    return eval(mod_name).get_available_datasets()# eval map a object
    # dont kown? by wenhua,maybe return the pakage form the .detaset
    ...
    for split in ["TRAIN", "TEST", "SS_TRAIN", "TEST_DEBUG", "TRAIN_REAL", "TRAIN2", "TRAIN_SYN_SUP"]:
        for name in cfg.DATASETS.get(split, []):
            ...
            for _mod_name in _DSET_MOD_NAMES:
                if name in get_available_datasets(_mod_name):  #if lm_13_train in get-(lm_dataset_d2') is ture
